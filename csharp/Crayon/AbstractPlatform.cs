...
namespace Crayon
{
	internal abstract class AbstractPlatform
	{
		...
		public void Compile(BuildContext buildContext, string inputFolder, string baseOutputFolder, string nullableReadableByteCodeOutputPath)
		{
			...
			spriteSheetBuilder.Generate(filesToCopyOver, spriteSheetOpsStringArgs, spriteSheetOpsIntArgs, spriteSheetFiles);
			...
			foreach (string file in spriteSheetFiles.Keys)
			{
				...
				{
					throw new InvalidOperationException("Autogenerated sprite sheet files are overwriting existing files. No user code should be contained in a folder called _crayon_gen_files.");
				}
