...
TranslationHelper_globals['LIB:GAME:pygame'] = None
...
try:
  ...
  with open(os.devnull, 'w') as f:
    sys.stdout = f
    import pygame
  TranslationHelper_globals['LIB:GAME:pygame'] = pygame
except:
  ...
  for ev in evs:
    if ev.type == pygame.KEYDOWN or ev.type == pygame.KEYUP:
      if KEY_LOOKUP.get(ev.key) != None:
        ...
        evlist.append(buildRelayObj(16 if down else 17, keycode, 0, 0, 0, None))
      if ev.type == pygame.KEYDOWN and ev.key == pygame.K_F4:
        if pressed_keys[pygame.K_LALT] or pressed_keys[pygame.K_RALT]:
          evlist.append(buildRelayObj(1, 0, 0, 0, 0, None))
    elif ev.type == pygame.QUIT:
      evlist.append(buildRelayObj(1, 1, 0, 0, 0, None))
      ...
      if not down: type += 1
      if right: type += 2
      evlist.append(buildRelayObj(type, x, y, 0, 0, None))
    elif ev.type == pygame.MOUSEMOTION:
      x, y = ev.pos
      x = x * vwidth // rwidth
      y = y * vheight // rheight
      evlist.append(buildRelayObj(32, x, y, 0, 0, None))
...
def pygame_initialize_screen(width, height, pixel_dimensions, execId):
  ...
  while True:
    result = TH_runInterpreter(execId)
    if getVmResultStatus(result) != 2:
      break
    gfxRender()
...
def pygame_getCallbackFunctions():
  return {
    "set-render-data": lambda o: gfxRendererSetData(o[0], o[1], o[2], o[3])
  }
