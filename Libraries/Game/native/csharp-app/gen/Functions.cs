...
{
    return CrayonWrapper.resource_manager_getResourceOfType(vm, (string)args[0].internalValue, "SND");
}
...
{
    return CrayonWrapper.buildBoolean(vm.globals, (lib_audio_music_playImpl((ObjectInstance)args[0].internalValue, (bool)args[1].internalValue, (string)args[2].internalValue, (double)args[3].internalValue, (bool)args[4].internalValue) != -1));
}
...
public static Value lib_audio_sfx_get_state(VmContext vm, Value[] args)
{
    ...
    return CrayonWrapper.buildInteger(vm.globals, lib_audio_sfx_get_stateImpl(nativeChannel, nativeSound, resourceId));
}
...
public static Value lib_audio_sfx_play(VmContext vm, Value[] args)
{
    ...
    return CrayonWrapper.buildInteger(vm.globals, lib_audio_sfx_launch(nativeResource, channelInstance.nativeData, vol, pan));
}
...
public static Value lib_game_gamepad_current_device_count(VmContext vm, Value[] args)
{
    ...
    return CrayonWrapper.buildInteger(vm.globals, total);
}
...
public static Value lib_game_gamepad_get_axis_1d_state(VmContext vm, Value[] args)
{
    ...
    return CrayonWrapper.buildFloat(vm.globals, Libraries.Game.GamepadTranslationHelper.GetDeviceAxis1dState(dev.nativeData[0], index));
}
...
public static Value lib_game_gamepad_get_axis_2d_state(VmContext vm, Value[] args)
{
    ...
    CrayonWrapper.clearList(list1);
    CrayonWrapper.addToList(list1, CrayonWrapper.buildInteger(vm.globals, PST_IntBuffer16[0]));
    CrayonWrapper.addToList(list1, CrayonWrapper.buildInteger(vm.globals, PST_IntBuffer16[1]));
    return vm.globalNull;
}
...
public static Value lib_game_gamepad_get_save_file_path(VmContext vm, Value[] args)
{
    ...
    if ((string1 != null))
    {
        return CrayonWrapper.buildString(vm.globals, string1);
    }
    return vm.globalNull;
}
...
{
    return CrayonWrapper.buildInteger(vm.globals, 1);
}
...
public static Value lib_game_gamepad_initialize_device(VmContext vm, Value[] args)
{
    ...
    CrayonWrapper.clearList(list1);
    CrayonWrapper.addToList(list1, CrayonWrapper.buildString(vm.globals, Libraries.Game.GamepadTranslationHelper.GetDeviceName(object1)));
    CrayonWrapper.addToList(list1, CrayonWrapper.buildInteger(vm.globals, Libraries.Game.GamepadTranslationHelper.GetDeviceButtonCount(object1)));
    CrayonWrapper.addToList(list1, CrayonWrapper.buildInteger(vm.globals, Libraries.Game.GamepadTranslationHelper.GetDeviceAxis1dCount(object1)));
    CrayonWrapper.addToList(list1, CrayonWrapper.buildInteger(vm.globals, Libraries.Game.GamepadTranslationHelper.GetDeviceAxis2dCount(object1)));
    return vm.globalNull;
}
...
{
    return CrayonWrapper.buildInteger(vm.globals, 0);
}
...
public static Value lib_game_getScreenInfo(VmContext vm, Value[] args)
{
    ...
    if (GameWindow.GetScreenInfo(o))
    {
        ...
        CrayonWrapper.clearList(output);
        CrayonWrapper.addToList(output, CrayonWrapper.buildBoolean(vm.globals, (o[0] == 1)));
        CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(vm.globals, o[1]));
        CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(vm.globals, o[2]));
    }
    return outList;
}
...
public static Value lib_game_getTouchState(VmContext vm, Value[] args)
{
    ...
    while ((i < end))
    {
        CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(vm.globals, data[i]));
        i += 1;
    }
    return vm.globalNull;
}
...
{
    GameWindow.FPS = CrayonWrapper.getFloat(args[0]);
    return vm.globalNull;
}

public static Value lib_game_initialize_screen(VmContext vm, Value[] args)
{
    ExecutionContext ec = CrayonWrapper.getExecutionContext(vm, CrayonWrapper.vm_getCurrentExecutionContextId(vm));
    GameWindow.InitializeScreen((int)args[0].internalValue, (int)args[1].internalValue, (int)args[2].internalValue, (int)args[3].internalValue, CrayonWrapper.vm_getCurrentExecutionContextId(vm));
    CrayonWrapper.vm_suspend_for_context(ec, 1);
    return vm.globalNull;
}
...
public static Value lib_game_pump_events(VmContext vm, Value[] args)
{
    ...
    if ((_len > 0))
    {
        ...
        while ((i < _len))
        {
            ...
            CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(globals, ev.type));
            int t = ev.type;
            CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(globals, ev.iarg1));
            if ((t >= 32))
            {
                CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(globals, ev.iarg2));
                if ((t == 37))
                {
                    CrayonWrapper.addToList(output, CrayonWrapper.buildFloat(globals, ev.farg1));
                }
                else
                {
                    if (((t >= 64) && (t < 80)))
                    {
                        CrayonWrapper.addToList(output, CrayonWrapper.buildInteger(globals, ev.iarg3));
                    }
                }
            }
            i += 1;
...
public static Value lib_game_startup(VmContext vm, Value[] args)
{
    ...
    while ((i < names.Length))
    {
        ...
        if ((fn != null))
        {
            CrayonWrapper.registerNamedCallback(vm, "Game", name, fn);
        }
        i += 1;
    }
    return vm.globalNull;
