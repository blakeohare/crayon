...
| **Method** | **Explanation** | **Complexity** |
| --- | --- | -- |
| `list.add(item, ...)` | Adds 1 or more items to `list`. Return value is `null`. | `O(1)` |
...
| `list.concat(otherList)` | Adds all items from `otherList` to the end of `list`. This mutates the original list. For a new list that is the concatenation of two lists, you can use the `+` operator between two lists to create a new combined list. | `O(otherList.length)` |
| `list.contains(item)` | Returns a boolean indicating whether the given item is present in the list. The condition of presence uses the same rules as `==`. | `O(n)` |
...
| `list.pop()` | Removes the last item from the list and returns it. This is a O(1) operation. | `O(1)` |
